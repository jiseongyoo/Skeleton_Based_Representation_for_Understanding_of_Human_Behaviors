...
int svm_scale_function(int argc,char argv[][1024])
{
	...
	scale::line = (char *) malloc(scale::max_line_len*sizeof(char));
...
#define SKIP_ELEMENT\
	...
	free(feature_min);
	fclose(fp);
	printf("Scaling has been finished\n");
	return 0;
}
...
char* scale::readline(FILE *input)
{
	...
	if(fgets(scale::line,scale::max_line_len,input) == NULL)
		return NULL;

	while(strrchr(scale::line,'\n') == NULL)
	{
		scale::max_line_len *= 2;
		scale::line = (char *) realloc(scale::line,scale::max_line_len);
		len = (int) strlen(scale::line);
		if(fgets(scale::line+len,scale::max_line_len-len,input) == NULL)
			break;
	}
	return scale::line;
}
...
void output(int index, double value, FILE *output)
{
	...
	if(value != 0)
	{
		fprintf(output, "%d:%g ",index, value);
		new_num_nonzeros++;
	}
}
